...
int main()
{
	...
	if (GetOpenFileName(&ofn))
	{
		...
		__try
		{
			...
			{
				pExport = (PIMAGE_EXPORT_DIRECTORY)((DWORD_PTR)virtualpointer + \
				...
				while (pExport->AddressOfNames != NULL)
				{
					libname[i] = (PCHAR)((DWORD_PTR)virtualpointer + RvaToOffset(pExport->AddressOfNames, pSech, ntheaders));
					cout << "\t" << libname[i] << endl;
					pExport++;
					i++;
